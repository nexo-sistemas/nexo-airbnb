...
Route::group([
    ...
    Route::get('/frm', function (Request $request) {
        $entidad = DB::select("SELECT entidad.uuid FROM entidad WHERE id = (SELECT usuario_entidad.entidad_id FROM usuario_entidad WHERE usuario_entidad.user_id = (SELECT users.id FROM users WHERE users.uuid = ? ) )",[$request->u]);
        return view('pages.ficha', [
            'key' => $entidad[0]->uuid,
            'unidades' => DB::select(
                "SELECT id,departamento FROM unidad_inmobiliaria
                WHERE unidad_inmobiliaria.estado = TRUE AND unidad_inmobiliaria.propietario = (SELECT id FROM users WHERE users.uuid = ?)",
                [$request->u]
            )
        ]);
    });
